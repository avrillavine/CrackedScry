...
public class SAMeshColliderEditorCommon
{
	...
	public class MeshCache
	{
		...
		public Vector3[] TriangleNormals { get; private set; }
		...
		public MeshCache( Mesh mesh, Material[] materials )
		{
			...
			Triangles = new int[subMeshTriangleCount];
			TriangleNormals = new Vector3[subMeshTriangleCount / 3];
			int subMeshTriangleIndex = 0;
			for( int subMesh = 0; subMesh < subMeshCount; ++subMesh ) {
				if( _subMeshes[subMesh].triangles != null ) {
					System.Array.Copy( _subMeshes[subMesh].triangles, 0, Triangles, subMeshTriangleIndex, _subMeshes[subMesh].triangles.Length );
					System.Array.Copy( _subMeshes[subMesh].triangleNormals, 0, TriangleNormals, subMeshTriangleIndex / 3, _subMeshes[subMesh].triangleNormals.Length );
						...
						if( processCount > 0 ) {
							...
							for( int i = 0; i < processCount; ++i ) {
								ComputeNormalsTask task = new ComputeNormalsTask
								{
									...
									subMesh = subMesh,
									triangleBegin = triangleBegin,
									triangleEnd = Mathf.Min(triangleBegin + processInTriangles, triangles.Length)
								};
								tasks.Add(task);
	...
	public static SplitMesh[] MakeSplitMeshesByMaterial( MeshCache meshCache )
	{
		...
		for( int subMesh = 0; subMesh < subMeshCount; ++subMesh ) {
			...
			if( triangles != null ) {
				...
				{
					subMeshCount = meshCache.SubMeshCount,
					subMesh = subMesh
				};
		...
		if( triangles != null ) {
			...
			for( int triangle = 0; triangle < triangles.Length; triangle += 3 ) {
				if( !usedTriangles[triangle / 3] ) {
					...
					{
						subMeshCount = meshCache.SubMeshCount,
						subMesh = splitMesh.subMesh,
						triangle = triangle,
						triangleVertex = vertices[triangles[triangle + 0]]
					};
	...
	public static SplitMesh[] MakeSplitMeshesByPolygon( MeshCache meshCache, SplitMesh splitMesh, float splitPolygonNormalAngle )
	{
		...
		for( int triangle = 0; triangle < triangles.Length; triangle += 3 ) {
			if( !usedTrianglesAll[triangle / 3] ) {
				...
				SplitMesh resplitMesh = new SplitMesh
				{
					subMeshCount = splitMesh.subMeshCount,
					subMesh = splitMesh.subMesh,
					triangle = splitMesh.triangle
				};
	...
	public static void RegistReducerTask( List<ReducerTask> reducerTasks, SAMeshCollider meshCollider )
	{
		...
		ReducerTask reducerTask = new ReducerTask
		{
			...
			splitMesh = splitMesh,
			reducerProperty = reducerProperty
		};
		...
		bool isDebug)
	{
		...
		if( reducerProperty.shapeType == ShapeType.Mesh ) {
			Mesh mesh = new Mesh
			{
				vertices = reducerResult.vertices,
				triangles = reducerResult.triangles
			};
	...
	public static ReducerResult PostfixReducerVertices( ReducerParams reducerParams, Vector3[] vertices, int[] triangles )
	{
		...
		{
			ReduceMode1 = SAColliderBoxReducer.ReduceMode.Box,
			VertexList = vertices,
			OptimizeRotation = reducerParams.reducerProperty.optimizeRotation,
			PostfixTransform = false
		};
		reducer.Reduce();
	...
	public static ReducerResult Reduce( ReducerParams reducerParams )
	{
		...
		if( reducerParams.reducerProperty.shapeType == ShapeType.Mesh &&
			...
			{
				mFlags = SAColliderHullReducer.HullFlag.QF_TRIANGLES,
				mVcount = triangles.Length
			};
			for ( int i = 0; i < triangles.Length; ++i ) {
			...
			SAColliderBoxReducer reducer = new SAColliderBoxReducer
			{
				ReduceMode1 = (SAColliderBoxReducer.ReduceMode)reducerParams.reduceMode,
				VertexList = reducerParams.vertices,
				LineList = reducerParams.lines,

				SliceCount = Mathf.Max((reducerProperty.maxTriangles - 4) / 8, 1),

				SliceMode1 = (SAColliderBoxReducer.SliceMode)(int)reducerProperty.sliceMode,
				MinThickness = reducerProperty.minThickness,
				OptimizeRotation = reducerProperty.optimizeRotation,
				Scale = reducerProperty.scale,
				Offset = reducerProperty.offset,
				ThicknessA = reducerProperty.thicknessA,
				ThicknessB = reducerProperty.thicknessB,
				PostfixTransform = false
			};
			...
			ReducerResult reducerResult = new ReducerResult
			{
				...
				vertices = reducer.ReducedVertexList,
				triangles = reducer.ReducedIndexList
			};
			return reducerResult;
		}
	}
