	...
	*/
		...
		BoneMeshCache boneMeshCache )
	{
		...
		ReducerTask reducerTask = new ReducerTask
		{
			...
			boneMeshCache = boneMeshCache,
			reducerProperty = boneCollider.reducerProperty
		};
	...
	public static ReducerResult PostfixReducerVertices( ReducerTask reducerTask, Vector3[] vertices, int[] triangles )
	{
		...
		SAColliderBoxReducer reducer = new SAColliderBoxReducer
		{
			...
			VertexList = vertices,
			OptimizeRotation = reducerTask.reducerProperty.optimizeRotation,
	...
	static void ReduceBoneCollider( ReducerTask reducerTask )
	{
		...
		if( reducerProperty.shapeType == ShapeType.Mesh &&
			...
			SAColliderBoxReducer reducer = new SAColliderBoxReducer
			{
				ReduceMode1 = (SAColliderBoxReducer.ReduceMode)reduceMode,
				VertexList = reducerTask.vertices,
				LineList = SAColliderBuilderEditorCommon.TriangleToLineIndices(reducerTask.triangles),
			...
			ReducerResult reducerResult = new ReducerResult
			{
				...
				colliderToChild = reducerTask.colliderToChild
			};
			reducerTask.reducerResult		= reducerResult;
		}
	}
	...
	private class BoneMeshCreator
	{
		...
		bool _Process()
		{
							...
							for( int n = 0; n < 4; ++n ) {
								if( boneIndexArray[n] >= 0 ) {
									if( boneWeightArray[n] > greaterBoneWeight ) {
										...
										greaterBoneIsTarget = _targetBones[greaterBoneIndex];
			...
			bool[] passedVertex = _boneMeshCache.PassedVertices;
			...
			int[] redirectIndex = _boneMeshCache.RedirectIndices;
			for( int i = 0, index = 0; i < _meshVertexCount; ++i ) {
				if( passedVertex[i] ) {
